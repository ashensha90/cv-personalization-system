        ...
        with snip_path.open("r", encoding="utf-8") as f:
            ...
            for line in f:
                ...
                try:
                    ...
                    st.warning(f"⚠️ Missing required field on line {line_num}: {str(e)}")
                    continue
        
        if items:
            coll.add(ids=ids, documents=items, metadatas=metas)
        else:
            st.error("❌ No valid snippets found in snippets.jsonl")
    return coll

def _overlap_score(snippet_meta: Dict, jd_skills: Set[str]) -> float:
    tags_str = (snippet_meta or {}).get("tags", "")
    # Convert comma-separated string back to set
    if isinstance(tags_str, str):
        tags = set(t.strip().lower() for t in tags_str.split(",") if t.strip())
    else:
        tags = set()
...
# ---------- Helper for index conversion ----------
def _safe_int(val):
    """Convert value to int, handling strings and returning None for invalid values."""
    try:
        return int(val)
    except (ValueError, TypeError):
        return None

def _safe_indices(indices_list):
    """Convert a list of indices (which may be strings) to integers, filtering out invalid ones."""
    result = []
    for idx in indices_list:
        safe_idx = _safe_int(idx)
        if safe_idx is not None:
            result.append(safe_idx)
    return result
...
def write_resume_docx(cv: Dict, path: Path = None) -> BytesIO:
    ...
    if "template" in cv and "selection" in cv:
        ...
        if summary_section:
            ...
            bullets_to_keep = _safe_indices(selection.get("summary_bullets_to_keep", []))
        ...
        if experience_list:
            ...
            if not experience_selection:
                # If no selection provided, include all experiences with all bullets
                ...
                for exp_sel in experience_selection:
                    ...
                    bullets = exp.get("bullets", [])
                    bullets_to_keep = _safe_indices(exp_sel.get("bullets_to_keep", []))
                    
                    # If no bullets specified, include all
                    if not bullets_to_keep:
                        bullets_to_keep = list(range(len(bullets)))
                    ...
                    achievements = exp.get("achievements", [])
                    achievements_to_keep = _safe_indices(exp_sel.get("achievements_to_keep", []))
                    
                    # If no achievements specified but achievements exist, include all
                    if not achievements_to_keep and achievements:
                        achievements_to_keep = list(range(len(achievements)))
        ...
        training_to_keep = _safe_indices(selection.get("training_and_education_to_keep", []))
...
- ALWAYS include at least 2-3 experience entries with relevant bullets
- Prefer the most senior and most recent experience when choosing what to keep.
- Array indices must be integers (0, 1, 2), NOT strings ("0", "1", "2")
...
Output ONLY a JSON object in this exact schema. Use integers for indices, not strings:

{{
  "summary_bullets_to_keep": [0, 1],

  "experience": [
    {{
      "id": "insead",
      "include": true,
      "bullets_to_keep": [0, 1, 2, 3],
      "achievements_to_keep": [0, 1, 2]
    }},
    {{
      "id": "moqdigital",
      "include": true,
...
CRITICAL: 
- You MUST include at least 2-3 experience entries with "include": true
- For each included experience, provide specific bullet indices as integers
- Indices start at 0 (first bullet is 0, second is 1, etc.)
"""
...
with col_info1:
    company_name = st.text_input(
        ...
        placeholder="Will auto-populate from JD if left empty",
        key="company_input"
    )
with col_info2:
...
jd_text = st.text_area("Paste the full Job Description here:", height=280, 
...
if gen_clicked:
    ...
    with st.spinner("🔄 Parsing JD and retrieving snippets..."):
        ...
        # Auto-populate company and title from JD if not provided
        if not company_name and jd.get("company"):
            st.session_state["company_name"] = jd.get("company")
            company_name = jd.get("company")
        elif company_name:
            st.session_state["company_name"] = company_name
            
        if not job_title and jd.get("title"):
            st.session_state["job_title"] = jd.get("title")
            job_title = jd.get("title")
        elif job_title:
            st.session_state["job_title"] = job_title

    # Show extracted/provided info
    st.info(f"📌 **Applying for:** {job_title or 'N/A'} at **{company_name or 'N/A'}**")
    
    if show_debug:
        with st.expander("🔍 Debug Info"):
            st.caption(f"**JD Hash:** {jd_hash[:12]}...")
            st.write(f"**Title (from JD):** {jd.get('title', 'N/A')}")
            st.write(f"**Company (from JD):** {jd.get('company', 'N/A')}")
            st.write(f"**Title (using):** {job_title or 'N/A'}")
            st.write(f"**Company (using):** {company_name or 'N/A'}")
    ...
    with st.expander("📋 Selection JSON", expanded=False):
        st.json(selection)
        
        # Show summary of what's included
        exp_sel = selection.get("experience", [])
        included_count = sum(1 for e in exp_sel if e.get("include", True))
        st.info(f"**Experience entries included:** {included_count}/{len(exp_sel)}")
        
        for exp in exp_sel:
            if exp.get("include", True):
                exp_id = exp.get("id", "unknown")
                bullets_count = len(exp.get("bullets_to_keep", []))
                achievements_count = len(exp.get("achievements_to_keep", []))
                st.text(f"✓ {exp_id}: {bullets_count} bullets, {achievements_count} achievements")
